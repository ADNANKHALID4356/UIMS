...
/**
 * FarmersPage - Complete offline farmer management UI
 * Implements SRS FR-3.1 requirements (Sprint 2)
 */
const FarmersPage = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { farmers, selectedFarmer, loading, error } = useSelector((state) => state.farmer);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (searchTerm.trim()) {
        dispatch(searchFarmers(searchTerm));
      } else {
        dispatch(fetchAllFarmers(true));
      }
    }, 500);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modalMode === 'create') {
        await dispatch(createFarmer({ farmerData: formData, userId: user.user_id })).unwrap();
      } else {
        await dispatch(
          updateFarmer({
  ...
  const handleDeleteConfirm = async () => {
    try {
      await dispatch(
        deleteFarmer({ farmerId: farmerToDelete.farmer_id, userId: user.user_id })
      ).unwrap();
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Farmer Management</h1>
            <p className="text-gray-600">Manage farmer records completely offline</p>
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div className="text-center p-12">
              ...
              <p className="mt-1 text-sm text-gray-500">
                {searchTerm ? 'Try a different search term' : 'Get started by creating a new farmer'}
              </p>
            </div>
          ) : (
                  ...
                  {farmers.map((farmer) => (
                    <tr key={farmer.farmer_id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        {farmer.father_name && (
                          <div className="text-sm text-gray-500">S/O {farmer.father_name}</div>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {formatCNIC(farmer.cnic)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {farmer.phone || '-'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <span
                          className={`font-medium ${
                            farmer.balance > 0 ? 'text-green-600' : 'text-gray-900'
                          }`}
                        >
                          Rs. {parseFloat(farmer.balance || 0).toFixed(2)}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <span
                          className={`font-medium ${
                            farmer.credit > 0 ? 'text-red-600' : 'text-gray-900'
                          }`}
                        >
                          Rs. {parseFloat(farmer.credit || 0).toFixed(2)}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex gap-2">
                          <button
                            onClick={() => navigate(`/ledger/farmer/${farmer.farmer_id}`)}
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-gray-800">
                  {modalMode === 'create' ? 'Add New Farmer' : 'Edit Farmer'}
                </h2>
                <button
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="mt-6 flex gap-3 justify-end">
                  ...
                  >
                    {loading ? 'Saving...' : modalMode === 'create' ? 'Create Farmer' : 'Update Farmer'}
                  </button>
                </div>
              </form>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-sm text-gray-500 text-center mb-6">
              ...
              {farmerToDelete?.specific_id})? This action cannot be undone.
            </p>
